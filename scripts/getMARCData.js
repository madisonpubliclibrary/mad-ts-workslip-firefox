(function() {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (loginError !== null) {
      ...
      reject('Not logged into Bibliovation.')
    } else {
      let marcTimeout = 40; // 40 * 400ms = 16sec
      let waitForMARC = setInterval(() => {
        ...
        if (isbnArr.length > 0) marcData['020'] = isbnArr.map(x => x.match(/^(?:\d+)?/)[0]).filter(y => y !== '');
  
        if ((marcData.hasOwnProperty('001') && (marcData.hasOwnProperty('092') || marcData.hasOwnProperty('099a')
             || marcData.hasOwnProperty('300'))) || marcTimeout === 0) {
          clearInterval(waitForMARC);
          resolve(marcData);
        }
      }, 400);
